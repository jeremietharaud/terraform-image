...
RUN apk add --update curl jq unzip coreutils python3 py3-pip && \
    ...
    curl $(curl -sL https://releases.hashicorp.com/terraform/index.json | \
    jq -r '.versions[].builds[].url' | egrep 'terraform_0\.11\.14_linux.*amd64' | \
    sort -V | tail -1) > terraform.zip && unzip terraform.zip -d /usr/bin && \
